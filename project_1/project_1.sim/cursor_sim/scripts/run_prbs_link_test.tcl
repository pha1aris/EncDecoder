######################################################################
# run_prbs_link_test.tcl
# PRBS 链路建立测试脚本
# 使用改进版本的模块，测试链路建立和 PRBS 验证
######################################################################
...
# 设置输出目录
set work_dir [file join $cursor_sim_dir work_prbs_link]
set log_dir [file join $cursor_sim_dir logs]
file mkdir $work_dir
file mkdir $log_dir

# 设置日志文件
set log_file [file join $log_dir run_prbs_link_test.log]
set compile_log [file join $log_dir compile_prbs_link.log]
set elaborate_log [file join $log_dir elaborate_prbs_link.log]
set simulate_log [file join $log_dir simulate_prbs_link.log]

# 打开日志
set log_fp [open $log_file w]
puts $log_fp "=========================================="
puts $log_fp "PRBS Link Establishment Test Started"
...
set xelab_cmd [list xelab --debug typical --relax --mt 32 -L xil_defaultlib -L fifo_generator_v13_2_11 -L gtwizard_ultrascale_v1_7_19 -L xilinx_vip -L unisims_ver -L unimacro_ver -L secureip -L xpm --snapshot tb_prbs_link_establishment_behav xil_defaultlib.tb_prbs_link_establishment xil_defaultlib.glbl]
...
# ============================================================
# Step 3: 运行仿真（100us，用于链路建立和PRBS验证）
# ============================================================
puts "Running simulation (100us PRBS link test)..."

set sim_cmd_file [file join $work_dir sim_cmd.tcl]
set sim_cmd_fp [open $sim_cmd_file w]
puts $sim_cmd_fp "run 100us"
puts $sim_cmd_fp "quit"
close $sim_cmd_fp

set xsim_cmd [list xsim tb_prbs_link_establishment_behav -t sim_cmd.tcl -log $simulate_log]
...
puts $log_fp "\n=========================================="
puts $log_fp "PRBS Link Test Completed"
puts $log_fp "Time: [clock format [clock seconds]]"
puts $log_fp "=========================================="
close $log_fp
