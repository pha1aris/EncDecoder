######################################################################
# run_s4.tcl
# 用 Vivado xsim 批处理跑 S4 testbench (tb_fec_loopback_s4)
# fec_gth_loopback_top_s2 整体测试，目标是 10ms 仿真
######################################################################
...
# 设置输出目录
set work_dir [file join $cursor_sim_dir work_s4]
set log_dir [file join $cursor_sim_dir logs]
file mkdir $work_dir
file mkdir $log_dir

# 设置日志文件
set log_file [file join $log_dir run_s4.log]
set compile_log [file join $log_dir compile_s4.log]
set compile_vhdl_log [file join $log_dir compile_s4_vhdl.log]
set elaborate_log [file join $log_dir elaborate_s4.log]
set simulate_log [file join $log_dir simulate_s4.log]

# 打开日志
set log_fp [open $log_file w]
puts $log_fp "=========================================="
puts $log_fp "S4 Simulation Run Started"
...
# Step 3: 运行仿真（10ms）
# ============================================================
puts "Running simulation (10ms - this may take a while)..."

# 创建仿真命令文件
# 仿真时间 10ms = 10_000_000 ns
set sim_cmd_file [file join $work_dir sim_cmd.tcl]
set sim_cmd_fp [open $sim_cmd_file w]
puts $sim_cmd_fp "run 10ms"
puts $sim_cmd_fp "quit"
close $sim_cmd_fp

set xsim_cmd [list xsim tb_fec_loopback_s4_behav -t sim_cmd.tcl -log $simulate_log]
...
puts $log_fp "\n=========================================="
puts $log_fp "S4 Simulation Run Completed"
puts $log_fp "Time: [clock format [clock seconds]]"
puts $log_fp "=========================================="
close $log_fp
