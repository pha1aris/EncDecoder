######################################################################
# run_s2.tcl
# 用 Vivado xsim 批处理跑 S2 testbench (tb_fec_gth_loopback_top_s2)
# 自动添加需要的 sim 源文件（只引用，不复制）
######################################################################
...
# 设置输出目录
set work_dir [file join $cursor_sim_dir work_s2]
set log_dir [file join $cursor_sim_dir logs]
file mkdir $work_dir
file mkdir $log_dir

# 设置日志文件
set log_file [file join $log_dir run_s2.log]
set compile_log [file join $log_dir compile_s2.log]
set compile_vhdl_log [file join $log_dir compile_s2_vhdl.log]
set elaborate_log [file join $log_dir elaborate_s2.log]
set simulate_log [file join $log_dir simulate_s2.log]

# 打开日志
set log_fp [open $log_file w]
puts $log_fp "=========================================="
puts $log_fp "S2 Simulation Run Started"
...
set ip_files [list \
    ...
    [file join $project_root project_1.gen sources_1 ip fifo_0 sim fifo_0.v] \
]
...
set rtl_files [list \
    ...
    [file join $project_root project_1.srcs sources_1 new rs_deinterleaver_xpm.v] \
...
# 如果还是找不到，尝试从 Vivado 安装目录获取（需要知道 Vivado 路径）
...
# 将 glbl.v 复制到工作目录（如果源文件存在且目标不存在）
...
# ============================================================
# Step 1.5: 编译 VHDL IP 文件（rs_encoder_0 和 rs_decoder_0）
# ============================================================
puts "Compiling VHDL IP sources..."
...
# 构建 xvhdl 命令列表
set xvhdl_cmd [list xvhdl --relax -work xil_defaultlib]

# 添加所有 VHDL 文件到编译命令
foreach file $vhdl_ip_files {
    if {[file exists $file]} {
        lappend xvhdl_cmd $file
    }
}

# 执行 VHDL 编译
if {[llength $vhdl_ip_files] > 0} {
    set vhdl_compile_result [catch {
        set vhdl_compile_fp [open $compile_vhdl_log w]
        puts $vhdl_compile_fp "Executing: [join $xvhdl_cmd { }]"
        flush $vhdl_compile_fp
        exec {*}$xvhdl_cmd >@ $vhdl_compile_fp 2>@ $vhdl_compile_fp
        close $vhdl_compile_fp
    } vhdl_compile_error]
    
    if {$vhdl_compile_result != 0} {
        set log_fp [open $log_file a]
        puts $log_fp "ERROR: VHDL Compilation failed!"
        puts $log_fp $vhdl_compile_error
        close $log_fp
        exit 1
    }
}
...
set xelab_cmd [list xelab --debug typical --relax --mt 32 -L xil_defaultlib -L fifo_generator_v13_2_11 -L gtwizard_ultrascale_v1_7_19 -L xilinx_vip -L unisims_ver -L unimacro_ver -L secureip -L xpm -L rs_toolbox_v9_0_14 -L rs_decoder_v9_0_23 -L rs_encoder_v9_0_22 --snapshot tb_fec_gth_loopback_top_s2_behav xil_defaultlib.tb_fec_gth_loopback_top_s2 xil_defaultlib.glbl]
...
# 创建仿真命令文件
# 设置运行时间限制（例如 1ms 仿真时间），避免无限运行
# 如果需要更长的仿真时间，可以修改这里的值，例如：run 10ms, run 100ms 等
...
set xsim_cmd [list xsim tb_fec_gth_loopback_top_s2_behav -t sim_cmd.tcl -log $simulate_log]
...
if {[file exists $compile_vhdl_log]} {
    puts $log_fp "\n=========================================="
    puts $log_fp "Compilation Log (VHDL):"
    puts $log_fp "=========================================="
    set vhdl_compile_fp [open $compile_vhdl_log r]
    puts $log_fp [read $vhdl_compile_fp]
    close $vhdl_compile_fp
}
...
puts $log_fp "\n=========================================="
puts $log_fp "S2 Simulation Run Completed"
puts $log_fp "Time: [clock format [clock seconds]]"
puts $log_fp "=========================================="
close $log_fp
