######################################################################
# run_s1.tcl
# 用 Vivado xsim 批处理跑 S1 testbench (tb_fec_gth_loopback_top)
...
# 设置输出目录
set work_dir [file join $cursor_sim_dir work_s1]
set log_dir [file join $cursor_sim_dir logs]
file mkdir $work_dir
file mkdir $log_dir

# 设置日志文件
set log_file [file join $log_dir run_s1.log]
set compile_log [file join $log_dir compile_s1.log]
set elaborate_log [file join $log_dir elaborate_s1.log]
set simulate_log [file join $log_dir simulate_s1.log]

# 打开日志
set log_fp [open $log_file w]
puts $log_fp "=========================================="
puts $log_fp "S1 Simulation Run Started"
...
set rtl_files [list \
    ...
    [file join $project_root project_1.srcs sources_1 new tb_fec_gth_loopback_top.v] \
]
...
if {$compile_result != 0} {
    set log_fp [open $log_file a]
    puts $log_fp "ERROR: Compilation failed!"
...
set xelab_cmd [list xelab --debug typical --relax --mt 32 -L xil_defaultlib -L fifo_generator_v13_2_11 -L gtwizard_ultrascale_v1_7_19 -L xilinx_vip -L unisims_ver -L unimacro_ver -L secureip -L xpm --snapshot tb_fec_gth_loopback_top_s1_behav xil_defaultlib.tb_fec_gth_loopback_top xil_defaultlib.glbl]
...
set xsim_cmd [list xsim tb_fec_gth_loopback_top_s1_behav -t sim_cmd.tcl -log $simulate_log]

# 设置超时（例如 30 分钟），避免脚本无限等待
set simulate_result [catch {
    # 在 Windows 上，使用 timeout 命令限制执行时间
    # 注意：这需要 PowerShell 或使用其他方法
    exec {*}$xsim_cmd
} simulate_error]
...
puts $log_fp "\n=========================================="
puts $log_fp "S1 Simulation Run Completed"
puts $log_fp "Time: [clock format [clock seconds]]"
puts $log_fp "=========================================="
close $log_fp
