...
module frame_synchronizer_top #(
    ...
    parameter OUTPUT_DELAY      = 3,       //将数据延迟OUTPUT_DELAY 与sof分离
    ...
    input wire  [PARALLEL/8-1:0]      din_charisk,
    ...
    //延迟OUTPUT_DELAY 后的数据
    output reg [PARALLEL-1:0]         data_o,
  ...
  assign data_valid_internal = din_valid && (din_charisk == 'd0);
  ...
  wire        will_wrap       = data_valid_internal && (fcnt4_plus >= FRAME_LEN_BYTE);
  wire        frame_end_raw   = will_wrap;
  reg  [FRAME_CNT_W-1:0] fcnt4_phase;
  wire        is_at_phase     = (fcnt4 == fcnt4_phase);
  wire        frame_start_raw = data_valid_internal && is_at_phase;

  // 帧头期（原始）信号（均受 din_valid 门控）
  wire        in_header_raw   = data_valid_internal && (fcnt4 < HEADER_LEN_BYTES);
  wire        header_end_raw  = data_valid_internal && (fcnt4 == (HEADER_LEN_BYTES - BYTES_PER_CYCLE)); // 24-4=20
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      // 优先级 3: 计数
      else if (data_valid_internal) begin
        fcnt4 <= fcnt4_plus;
      end
    end
  end
  ...
  wire fifo_srst;
  // wire wr_ok = data_valid_internal && !wr_rst_busy && wr_allowed && !full; 
  wire wr_ok;
  assign wr_ok =  data_valid_internal && !wr_rst_busy && !fifo_srst && wr_allowed && !full;
...
// ========= 写口帧首打标 =========
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (entering_lock_d1 || entering_protect || fifo_srst)
        start_pending <= 1'b0;
  ...
  wire frame_start_wr = wr_ok && start_pending;  
  ...
  wire fifo_overflow_condition;
  assign fifo_overflow_condition = full && (state != S_LOCK); // 除了lock下都溢出保护

  wire fifo_srst_req = ~rst_n | entering_protect | entering_lock_d1 | fifo_overflow_condition;

  reg fifo_srst_r;
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n)                     
      fifo_srst_r <= 1'b1;
    else if (fifo_srst_req)         
      fifo_srst_r <= 1'b1;  // 触发就置位
    else if (!wr_rst_busy && !rd_rst_busy)
      fifo_srst_r <= 1'b0;  // 直到两端 busy 都拉低再释放
  end

  assign fifo_srst = fifo_srst_r;
...
// 读侧“已见tag”标志：仅由 FIFO 读口事件驱动
  ...
  always @(*) begin
    next_state = state;
    if (fifo_overflow_condition) begin //fifo溢出强制进入search
        next_state = S_SEARCH;
    end else begin
    case (state)
      S_SEARCH:
