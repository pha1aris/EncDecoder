...
module gearbox_8to32_bs (
    ...
    input      [7:0]   in_data,
    input              in_valid,
    input              in_block_start,   // 来自 rs_interleaver_xpm.out_block_start
    // 若后面不反压，这里可以没有 in_ready

    output reg [31:0]  out_data,
    output reg         out_valid,
    output reg         out_block_start   // 该 32bit word 内是否包含块起始符号
);
    reg [1:0]  byte_cnt;
    reg [31:0] word_reg;
        ...
        if (rst) begin
            ...
            if (in_valid) begin
                ...
                if (byte_cnt == 2'd3) begin
                    out_data        <= {in_data, word_reg[23:0]}; // 最后一个字节放 MSB
                    out_valid       <= 1'b1;
